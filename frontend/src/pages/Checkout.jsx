...
function Checkout() {
  ...
  const discountAmount =
    promoApplied && PROMO_CODES[appliedPromoKey]
  ...
  const handleContinue = () => {
    ...
    if (!dropoffLocation) {
      setError("Please pin your drop-off location.");
      return;
    }
      ...
      <div className="min-h-screen bg-fav3 flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center pt-24">
          <div className="text-center space-y-4">
            <div className="w-20 h-20 rounded-full bg-white flex items-center justify-center text-4xl mx-auto shadow-sm">
              🛒
            </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-8">
            {[
              ...
              <div key={s.num} className="flex items-center gap-2">
                ...
                {i < 2 && <div className="w-6 h-0.5 rounded-full bg-fav3" />}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-3xl p-5 sm:p-7 shadow-sm space-y-5">
                <div>
                  <span className="text-xs font-semibold tracking-widest uppercase text-fav6">
                    Step 1
                  </span>
                  <h2 className="text-2xl font-bold text-fav5 mt-1">
                ...
                <div>
                  <label className="block text-xs font-semibold text-fav4 mb-2 tracking-wide uppercase">
                    ...
                    <span className="text-fav7 normal-case font-normal">
                      (Optional)
                    </span>
                  </label>
                  <textarea
                ...
                {/* GPS */}
                <div className="rounded-2xl border-2 border-fav1/25 bg-fav1/5 p-4 sm:p-5 space-y-3">
                  <div>
                    <p className="text-sm font-bold text-fav5">
                      Drop-off pin (required)
                    </p>
                    <p className="text-xs text-fav6 mt-1 leading-relaxed">
                      Allow location access so we can place your delivery on the
                      live map.
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={requestDropoffGeolocation}
                    disabled={geoLoading}
                  ...
                  {dropoffLocation && (
                    <p className="text-xs text-green-700 font-medium break-all">
                      ✓ Pinned: {dropoffLocation.lat.toFixed(5)}°N,{" "}
                      {dropoffLocation.lng.toFixed(5)}°E
                    </p>
                  )}
                  {geoHint && (
                    <p
                      className={`text-xs ${dropoffLocation ? "text-fav6" : "text-amber-800"}`}
                    >
                      {geoHint}
                    </p>
                  )}
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      type="button"
                      onClick={handlePromo}
                      className="px-5 py-3 bg-fav4 hover:bg-fav5 text-fav3 rounded-xl text-sm font-semibold transition-all duration-300 flex-shrink-0"
                    >
                      Apply
                    </button>
                  </div>
                  ...
                  <p className="text-xs text-fav7 mt-2">
                    Try: SAVE10, SAVE20, WELCOME5
                  </p>
                </div>
                  ...
                  {cart.map((item) => (
                    <div key={item._id} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold text-fav5 truncate">
                          {item.name}
                        </p>
                        <p className="text-xs text-fav6">× {item.quantity}</p>
                      </div>
                      <p className="text-xs font-bold text-fav4 flex-shrink-0">
