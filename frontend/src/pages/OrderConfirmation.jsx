...
function OrderConfirmation() {
  ...
  const { deliveryInfo, dropoffLocation, discountAmount, finalTotal, DELIVERY_FEE, SERVICE_FEE, paymentInfo } = state;

  const [loading, setLoading] = useState(false);
  const [error,   setError]   = useState("");

  // Derive payment display info from type and details
  const getPaymentDisplay = () => {
    const { type, details } = paymentInfo;
    
    if (type === "cash") {
      return { icon: "💵", label: "Cash on Delivery" };
    }
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      clearCart();
      toast.success(`Order placed! 🎉 ${order.orderNumber}`, { duration: 6000 });
      navigate("/orders");
    } catch (err) {
          ...
          <div className="flex items-center gap-3 mb-10">
            {[
              ...
              <div key={s.num} className="flex items-center gap-3">
                ...
                {i < 2 && <div className={`w-8 h-0.5 rounded-full ${s.done ? "bg-fav4" : "bg-fav3"}`} />}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            <div className="lg:col-span-3 space-y-5">
              ...
              <div className="bg-white rounded-3xl p-6 shadow-sm space-y-4">
                ...
                <div className="space-y-3">
                  {cart.map((item) => (
                    <div key={item._id} className="flex items-center gap-3 py-2 border-b border-fav3/40 last:border-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-fav5 truncate">{item.name}</p>
                        <p className="text-xs text-fav6">× {item.quantity}</p>
                      </div>
                      <p className="text-sm font-bold text-fav4">${(item.price * item.quantity).toFixed(2)}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-white rounded-3xl p-6 shadow-sm space-y-3">
                <h3 className="font-bold text-fav5 text-lg">Delivery Details</h3>
                {[
                  ...
                  ...(deliveryInfo.deliveryInstructions
                    ? [{ icon: "📝", label: "Instructions", value: deliveryInfo.deliveryInstructions }]
                    : []),
                ].map((row) => (
                  <div key={row.label} className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-xs text-fav6 font-semibold uppercase tracking-wide">{row.label}</p>
                      <p className="text-sm text-fav5">{row.value}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="bg-white rounded-3xl p-6 shadow-sm">
                <h3 className="font-bold text-fav5 text-lg mb-4">Payment Method</h3>
                <div className="flex items-center gap-4 p-4 bg-fav3/50 rounded-2xl border border-fav3">
                  ...
                  <div>
                    <p className="font-bold text-fav5">{paymentDisplay.label}</p>
                    {paymentDisplay.lastFour && (
                      <p className="text-xs text-fav6">•••• •••• •••• {paymentDisplay.lastFour}</p>
                    )}
                    {paymentDisplay.phone && (
                      <p className="text-xs text-fav6">{paymentDisplay.phone}</p>
                    )}
                  </div>
            ...
            <div className="lg:col-span-2">
              <div className="bg-white rounded-3xl p-6 shadow-sm sticky top-24 space-y-5">
                <div>
                  <span className="text-xs font-semibold tracking-widest uppercase text-fav6">Final Summary</span>
                  <h2 className="text-xl font-bold text-fav5 mt-1">Price Breakdown</h2>
                </div>
