...
function Payment() {
  ...
  const validate = () => {
    if (!paymentType) { setError("Please select a payment method."); return false; }
    if (paymentType === "online") {
      if (onlineTab === "digital-wallet") {
        if (!wallet) { setError("Please select Easypaisa or JazzCash."); return false; }
        if (!walletPhone.trim() || walletPhone.replace(/\D/g,"").length < 10) {
          setError("Please enter a valid wallet phone number."); return false;
        }
      } else {
        if (cardData.cardNumber.replace(/\s/g,"").length < 16) { setError("Please enter a valid 16-digit card number."); return false; }
        if (!cardData.cardName.trim()) { setError("Cardholder name is required."); return false; }
        if (cardData.expiry.length < 5) { setError("Please enter a valid expiry (MM/YY)."); return false; }
        if (cardData.cvv.length < 3)    { setError("Please enter a valid 3-digit CVV."); return false; }
      }
    }
    return true;
  };
  ...
  const handleContinueToConfirmation = () => {
    ...
    const paymentInfo = {
      type: paymentType === "cash" ? "cash"
        : onlineTab === "digital-wallet" ? "digital-wallet"
        : onlineTab,
        ...
        <div className="w-full max-w-md">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
            ...
            <div className="flex justify-between items-center p-3 bg-amber-50 border border-amber-200 rounded-lg mb-5">
              ...
              <span className="text-lg font-bold text-amber-700">${finalTotal.toFixed(2)}</span>
            </div>
            ...
            <div className="mb-5">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { value: "online", label: "Pay Online", icon: "💳", sub: "Card or Wallet" },
                  { value: "cash",   label: "Cash on Delivery", icon: "💵", sub: "Pay on arrival" },
                ].map((opt) => (
                  <button
                    key={opt.value}
                    type="button"
                    onClick={() => { setPaymentType(opt.value); setError(""); }}
                  ...
                  >
                    ...
                    <p className={`text-xs font-bold ${paymentType === opt.value ? "text-amber-700" : "text-gray-700"}`}>
                      {opt.label}
                    </p>
                    <p className="text-xs text-gray-500">{opt.sub}</p>
                    {paymentType === opt.value && (
                      <span className="text-amber-600 text-xs font-bold">✓ Selected</span>
                    )}
                  </button>
                ))}
              </div>
            ...
            {paymentType === "online" && (
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-gray-500 font-semibold">Payment Details</span>
                  <div className="flex-1 h-0.5 bg-gray-200" />
                </div>
                ...
                <div className="flex gap-1 p-1 bg-gray-100 rounded-lg">
                  {[
                    ...
                    <button
                      key={tab.key}
                      type="button"
                      onClick={() => { setOnlineTab(tab.key); setError(""); }}
                ...
                {(onlineTab === "credit-card" || onlineTab === "debit-card") && (
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-2">Card Number</label>
                      <input
                        type="text" name="cardNumber"
                        value={cardData.cardNumber} onChange={handleCardChange}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={cardData.cardName} onChange={handleCardChange}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder="•••" maxLength={3} className={inputClass}
                        />
                      </div>
                    </div>
                    {/* Security note */}
                ...
                {onlineTab === "digital-wallet" && (
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
  {[
    { key: "easypaisa", label: "Easypaisa", img: easypaisaImg, activeColor: "border-green-500 bg-green-50", textColor: "text-green-700" },
    { key: "jazzcash",  label: "JazzCash",  img: jazzcashImg,  activeColor: "border-red-500 bg-red-50",     textColor: "text-red-700"   },
  ].map((w) => (
    <button
      key={w.key}
      type="button"
      onClick={() => { setWallet(w.key); setError(""); }}
    ...
    > 
      <img src={w.img} alt={w.label} className="w-10 h-10 object-contain" />
      <span className={`text-xs font-bold ${wallet === w.key ? w.textColor : "text-gray-700"}`}>
        {w.label}
      </span>
      {wallet === w.key && (
        <span className={`text-xs font-bold ${w.textColor}`}>✓</span>
      )}
    </button>
  ))}
</div>
                    ...
                    {wallet && (
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-2">
                          {wallet === "easypaisa" ? "Easypaisa" : "JazzCash"} Phone Number
                        </label>
                        <input
            ...
            {paymentType === "cash" && (
              <div className="flex items-center gap-3 p-4 bg-amber-50 border border-amber-200 rounded-lg mt-2">
                ...
                <div>
                  <p className="text-sm font-bold text-gray-700">Cash on Delivery</p>
                  <p className="text-xs text-gray-500 mt-0.5">
                    Please have <span className="font-bold text-amber-700">${finalTotal.toFixed(2)}</span> ready when your order arrives.
                  </p>
                </div>
              </div>
            )}
